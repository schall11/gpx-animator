...
L.TimeDimension = (L.Layer || L.Class).extend({
  ...
  initialize: function(options) {
    ...
    if (this._availableTimes.length > 0) {
      this.setCurrentTime(
        this.options.currentTime || this._getDefaultCurrentTime()
      );
    }
  ...
  setCurrentTimeIndex: function(newIndex) {
    ...
    var fullMin = d.substring(4, 24);
    // console.log("mon",mon);
    // console.log("yr",yr);
    //$('#dateDisp').text(d.substring(0,7));
    //$('#dateDisp').text(mon + " " + yr);
    $("#dateDisp").text(full);
    $("#dateDisp2").text(fullMin);
    if (
  ...
  _onSyncedLayerLoaded: function(e) {
    if (
      e.time == this._availableTimes[this._loadingTimeIndex] &&
      this._checkSyncedLayersReady(e.time)
    ) {
      this._newTimeIndexLoaded();
    }
  },
  ...
  setAvailableTimes: function(times, mode) {
    ...
    if (mode == "extremes") {
      var period = this.options.period || "P1D";
      this._availableTimes = L.TimeDimension.Util.explodeTimeRange(
        new Date(times[0]),
      ...
      if (this._availableTimes.length === 0) {
        ...
        this._availableTimes = L.TimeDimension.Util.intersect_arrays(
          parsedTimes,
          this._availableTimes
        );
      } else if (mode == "union") {
        this._availableTimes = L.TimeDimension.Util.union_arrays(
          parsedTimes,
          this._availableTimes
        );
  ...
  setLowerLimitIndex: function(index) {
    this._lowerLimit = Math.min(
      Math.max(index || 0, 0),
      this._upperLimit || this._availableTimes.length - 1
    );
  ...
  setUpperLimitIndex: function(index) {
    this._upperLimit = Math.max(
      Math.min(index, this._availableTimes.length - 1),
      this._lowerLimit || 0
    );
    this.fire("limitschanged", {
      lowerLimit: this._lowerLimit,
